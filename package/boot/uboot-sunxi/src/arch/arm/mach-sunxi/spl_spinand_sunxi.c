 ...
 *               2021 Robert Bartle <robert@axium.co.nz>
 */
...
struct egon_bt0_header {
	...
	union {
		...
		struct {
			const u8 spl_signature[4]; /* SPLv */
			u32 fel_script_address;
			u32 fel_uEnv_length;
			u32 dt_name_offset;
			u32 reserved1;
			u32 boot_media;
...
// 	},
	{
		.name = "Micron MT29F2G01ABAGDWB",
		.jedec_id = 0x002C242C, /* 3.3v */
		.page_shift = 11,
		.addr_mask = 0x7FF,
		.addr_shift = 0,
		.block_size = 128 * 1024,
	...
	{
		.name = "Micron MT29F1G01ABAFDWB",
		.jedec_id = 0x002C142C, /* 3.3v */
		.page_shift = 11,
		.addr_mask = 0x7FF,
		.addr_shift = 0,
		.block_size = 128 * 1024,
...
static void sunxi_spi0_load_page(struct sunxi_nand_config * config, u32 addr, ulong spi_ctl_reg,
	...
	/* Send the Read Page (13h) command header */
	...
	do {
		...
		/* Send the Get Status Bytes (0FC0h) command header */
...
static void sunxi_spi0_read_data(struct sunxi_nand_config * config, u8 *buf, u32 addr, u32 bufsize,
				...
				ulong spi_tc_reg,
				ulong spi_bcc_reg) {
	u8 plane_select_bit = config->plane_select && ((addr / config->block_size) % 2);
	addr = ((addr >> config->addr_shift) & config->addr_mask);
	if (plane_select_bit)
		addr |= 0x1000;
	...
	/* Send the Read From Cache (0Bh) command header */
...
static void spi0_read_data(struct sunxi_nand_config * config, void *buf, u32 addr, u32 len) {
	...
	while (len > 0) {
		curr_page = (addr >> config->page_shift);
		if (curr_page > last_page) {
			spi0_load_page(config, addr);
			last_page = curr_page;
		}

		chunk_len = (len >= SPI_READ_MAX_SIZE) ? SPI_READ_MAX_SIZE : len;
		if (((addr + chunk_len) >> config->page_shift) > curr_page) {
...
static void sunxi_spi0_reset(ulong spi_ctl_reg,
				ulong spi_ctl_xch_bitmask,
				ulong spi_fifo_reg,
				ulong spi_tx_reg,
				ulong spi_rx_reg,
				...
				ulong spi_tc_reg,
				ulong spi_bcc_reg) {
	/* Reset NAND to known state */
	writel(1, spi_bc_reg); /* Burst counter (total bytes) */
	writel(1, spi_tc_reg); /* Transfer counter (bytes to send) */
	if (spi_bcc_reg)
		writel(1, spi_bcc_reg); /* SUN6I also needs this */
		
	/* Send the Reset (FFh) command header */
	writeb(0xFF, spi_tx_reg);
	
	/* Start the data transfer */
	setbits_le32(spi_ctl_reg, spi_ctl_xch_bitmask);
	
	while ((readl(spi_fifo_reg) & 0x7F) < 1)
		;

	/* Discard the empty byte from our send */
	readb(spi_rx_reg);
	
	/* Power-on reset time specified as up to 1.25ms */
	udelay(1250);
}
...
static u32 sunxi_spi0_read_id(ulong spi_ctl_reg,
	...
	/* Send the Read ID (9Fh) command header */
...
static void print_egon_header(struct egon_bt0_header * egon) {
	...
	printf("Check: %x\n", egon->checksum);
	printf("Length: %x\n",egon->length);
	if (memcmp(egon->spl_signature, "SPL", 3) == 0) {
		printf("SPL version: %u\n", egon->spl_signature[3]);
		printf("FEL script addr: %x\n",egon->fel_script_address);
		printf("FEL uEnv lengthr: %x\n",egon->fel_uEnv_length);
		printf("DT name offset: %x\n",egon->dt_name_offset);
		printf("Reserved: %x\n", egon->reserved1);
		printf("Boot media: %x\n", egon->boot_media);
	} else {
		printf("Hdrsz: %u\n",egon->header_size);
...
static u32 spi0_count_spls(struct sunxi_nand_config * config, struct egon_bt0_header* self) {
	...
	for (entry_point = 0; entry_point < 8; ++entry_point) {
		...
		spi0_read_cache(config, buffer, address, EGON_BTO_HEADER_SIZE);
#ifdef DEBUG
		...
		if (strncmp(egon->magic, egon_bt0_magic, 8) == 0) {
			...
			if (hits > 2) { /* at least > 50% similarity */
				++spls;
			}
		}
	}
...
static int spl_spi_load_image(struct spl_image_info *spl_image,
	...
	/*
	 * If we receive only zeros, there is most definitely no device attached. 
	 */
	if (id == 0) {
		spi0_deinit();
		printf("%s: No spi-nand attached\n",spl_name);
		return -1;
	}
	...
	/*
	 * eGON.BRM loads the SPL that is running from the beginning of SRAM. 
	 * Being validated by eGON.BRM we can safely assume that all values are valid!
	 * Actually it seems the SPL reuses some of the bytes starting from the signature field.
	 */
	egon = (struct egon_bt0_header *)map_sysmem(CONFIG_SUNXI_SRAM_ADDRESS, EGON_BTO_HEADER_SIZE);
#ifdef DEBUG
	printf("Self: \n");
	print_egon_header(egon);
#endif
	spls = spi0_count_spls(config, egon);
	...
	do {
		printf("%s: Attempting to load u-boot from 0x%x\n", spl_name, uboot_offset);

		/*
		 * Read the header data from the image and parse it for validity.
		 */
		spi0_read_data(config, (void *)header, uboot_offset, sizeof(struct image_header));
		if (image_check_hcrc(header)) {
			...
			if (ret == 0) {
				...
				if (image_check_dcrc(header)) {
					...
					debug("%s: data length %d, checksum: %x, expected %x\n", spl_name, image_get_data_size(header), crc32(0, (u8*)image_get_data(header), image_get_data_size(header)), image_get_dcrc(header));
					ret = 1;
				}
			}
			...
			if (ret != 0) {
				/*
				 * The header was ok, so we can safely use the datasize to calculate the next PEB to look for u-boot
				 */
